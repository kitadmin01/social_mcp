...
class WorkflowGraph:
    ...
    async def post_to_linkedin_node(self, state):
        if state.get('error'):
            return state
        if not state.get('text'):
            error_msg = "No text content available for LinkedIn post generation"
            return {**state, "error": error_msg}
        current_row = state["current_row"]
        if current_row.get('sheet') != 'Sheet1':
            # Only post to LinkedIn for Sheet1
            return {**state, "skipped": True, "reason": "Not a LinkedIn URL"}
        text = state["text"]
        url = current_row.get('url', '')
        try:
            # Generate LinkedIn summary using LLM
            prompt = f"""Write a LinkedIn post summary for the following article. Include a call to action. Content: {text}\nLink: {url}"""
            summary = await self.llm.generate_content(prompt)
            if not summary:
                raise Exception("No summary generated by LLM")
            # Compose post: summary + link
            post_text = f"{summary}\n\nRead more: {url}"
            result = self.linkedin.create_post(post_text)
            # Update Sheet1 with LinkedIn result
            sheet = self.sheets.sheet.worksheet("Sheet1")
            headers = sheet.row_values(1)
            col_indices = {col: headers.index(col) + 1 for col in headers}
            sheet.update_cell(current_row['id'], col_indices.get('linkedin_result', len(headers)+1), json.dumps(result))
            return {**state, "linkedin_posted": True, "linkedin_result": result}
        except Exception as e:
            ...
            headers = sheet.row_values(1)
            col_indices = {col: headers.index(col) + 1 for col in headers}
            sheet.update_cell(current_row['id'], col_indices.get('linkedin_result', len(headers)+1), error_msg)
            return {**state, "error": error_msg}
    ...
    def build_workflow_graph(self):
        ...
        graph.add_edge("post_to_telegram", "post_to_linkedin")
        graph.add_edge("post_to_linkedin", "post_to_telegram")
