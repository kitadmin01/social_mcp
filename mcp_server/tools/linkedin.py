...
class LinkedInPoster:
    ...
    async def search_and_like_posts(self, search_term: str, like_count: int = 5) -> bool:
        """Search for posts on LinkedIn and like them.
        
        Args:
            search_term (str): Term to search for
            like_count (int, optional): Number of posts to like. Defaults to 5.
            
        Returns:
            bool: True if successful, False otherwise
        """
        try:
            logger.info(f"Searching LinkedIn for posts with term: {search_term}")
            
            # Prepare the API request for search
            search_url = 'https://api.linkedin.com/v2/search/ugcPosts'
            headers = {
                'Authorization': f'Bearer {self.access_token}',
                'X-Restli-Protocol-Version': '2.0.0',
                'Content-Type': 'application/json'
            ...
            params = {
                'q': search_term,
                'count': like_count * 2,  # Get more results than needed in case some can't be liked
                'start': 0
            }
            ...
            if not search_results.get('elements'):
                logger.warning("No posts found matching search term")
                return False
                
            # Like each post
            liked_count = 0
            for post in search_results['elements']:
                ...
                try:
                    ...
                    like_url = f'https://api.linkedin.com/v2/socialActions/{post_id}/likes'
                    like_data = {
                        "actor": f"urn:li:company:{self.company_id}",
                        "object": f"urn:li:ugcPost:{post_id}"
                    }
                    
                    # Make the like request
                    like_response = requests.post(like_url, headers=headers, json=like_data)
                    like_response.raise_for_status()
                    
                    liked_count += 1
                    logger.info(f"Liked post {post_id}")
                    
                    # Add a small delay between likes to avoid rate limiting
                    await asyncio.sleep(2)
                    
                except Exception as e:
                    logger.error(f"Error liking post {post.get('id')}: {str(e)}")
                    continue
            
            logger.info(f"Successfully liked {liked_count} posts on LinkedIn")
            return True
            
        except Exception as e:
            logger.error(f"Error in search_and_like_posts: {str(e)}")
            if hasattr(e, 'response'):
                logger.error(f"Response: {e.response.text}")
            return False 